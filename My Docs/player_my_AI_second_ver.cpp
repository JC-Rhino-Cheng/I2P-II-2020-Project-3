...
int OthelloBoard::MiniMax(int cur_depth, int terminal_depth) {
	...
	if (cur_depth == terminal_depth) {
		...
		if (cur_player == BLACK) {//分數越高代表對BLACK越好//這一行是針對BLACK的valid_to_be_put部分而已
			score += num_of_to_be_put_corners * 150 + num_of_to_be_put_corners_two_sides * 15 - num_of_to_be_put_corners_inner * 20 + num_of_to_be_put_remainings * 3;
		}
		...
		//所以這一部分是黑白都要考慮。
		score += num_of_putted_corners[BLACK] * 100 - num_of_putted_corners_inner[BLACK] * 15 + num_of_putted_corners_two_sides[BLACK] * 10 + num_of_putted_remainings[BLACK];
		score -= num_of_putted_corners[WHITE] * 100 - num_of_putted_corners_inner[WHITE] * 15 + num_of_putted_corners_two_sides[WHITE] * 10 + num_of_putted_remainings[WHITE];
