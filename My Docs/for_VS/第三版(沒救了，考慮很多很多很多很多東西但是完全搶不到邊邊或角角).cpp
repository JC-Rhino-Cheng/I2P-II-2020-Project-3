...
public:
	...
	enum SCORE_JIAQUAN {
		//Mobi的各項數值要比真正占到了要小，否則他會鼓勵一堆潛在的好局面，但是始終不下好局面
		Mobi_Corner = 5000,
		Mobi_Corner_Inner = -20000,
		Mobi_Corner_Two_Sides = 1500,
		Mobi_Edge = 2500,
		Mobi_Remainings = 75,
		Mobi_Overall = 2000,

		Corner_Chu_Qi = 550000,
		Corner_Mo_Qi = 850000,
		Corner_Inner = -35000,
		Corner_Two_Sides = 4000,
		Edge_Chu_Qi = 7500,
		Edge_Mo_Qi = 10000,
		Remainings = 150,
	...
	static const int Latter_Quarter_Threshold = SIZE * SIZE * 2 / 3;
...
private:
	...
	int get_how_many_can_flip(Point center) {
		...
		int JiaQuan_flip_num = 0;
		for (auto& p : list) {
			if (p == Corners[0] || p == Corners[1] || p == Corners[2] || p == Corners[3])
				JiaQuan_flip_num += 100;
			else if (p.x == 0 || p.x == SIZE - 1 || p.y == 0 || p.y == SIZE - 1)JiaQuan_flip_num += 80;
			else JiaQuan_flip_num += 25;
		}

		return JiaQuan_flip_num;
	}
public:
	//Constructor
	OthelloBoard() {
...
int OthelloBoard::MiniMax(int cur_depth, int terminal_depth) {
	...
	if (cur_depth == terminal_depth) {
		int score_Mobi = 0;
		int score_cur_situation_state[3] = { 0,0,0 };//編號0(EMPTY)廢棄
		std::vector<int>how_many_can_flip;
		...
		for (int i = 0; i < next_valid_spots.size(); i++) {
			if (next_valid_spots[i] == Corners[0] || next_valid_spots[i] == Corners[1] || next_valid_spots[i] == Corners[2] || next_valid_spots[i] == Corners[3])
				...
				score_Mobi += Mobi_Corner_Inner;
			else if (next_valid_spots[i].x == 0 || next_valid_spots[i].x == SIZE - 1) {
				if (next_valid_spots[i].y == 1 || next_valid_spots[i].y == SIZE - 2) score_Mobi += Mobi_Corner_Two_Sides;
				else score_Mobi += Mobi_Edge;
			}
			else if (next_valid_spots[i].y == 0 || next_valid_spots[i].y == SIZE - 1) {
				if (next_valid_spots[i].x == 1 || next_valid_spots[i].x == SIZE - 2) score_Mobi += Mobi_Corner_Two_Sides;
				else score_Mobi += Mobi_Edge;
			}
			else score_Mobi += Mobi_Remainings;

			how_many_can_flip.push_back(get_how_many_can_flip(next_valid_spots[i]));
		}
		score_Mobi += next_valid_spots.size()*Mobi_Overall;

		//先計算目前盤面上的Corner數。Corner數會造成決定性的影響
		int num_of_occupied_corners = 0;
		for (int i = 0; i < 4; i++) {
			if (board[Corners[i].x][Corners[i].y] == BLACK)num_of_occupied_corners++;
		}
		num_of_occupied_corners -= 2;//平移，盡可能讓沒有corner的垃圾局面消失
		num_of_occupied_corners *= 2;//把正和負的效果同時放大
		...
		for (int i = 0; i < SIZE; i++)for (int j = 0; j < SIZE; j++) {
			if (board[i][j] != EMPTY) {
				...
				if (p == Corners[0] || p == Corners[1] || p == Corners[2] || p == Corners[3]) {
					if (disc_count[BLACK] + disc_count[WHITE] >= Latter_Quarter_Threshold)score_cur_situation_state[board[i][j]] += Corner_Mo_Qi * num_of_occupied_corners;
					else score_cur_situation_state[board[i][j]] += Corner_Chu_Qi * num_of_occupied_corners;
				}
				else if (p == Corners_Inner[0] || p == Corners_Inner[1] || p == Corners_Inner[2] || p == Corners_Inner[3])score_cur_situation_state[board[i][j]] += Corner_Inner;
				else if (p.x == 0 || p.x == SIZE - 1) {
					if (p.y == 1 || p.y == SIZE - 2)score_cur_situation_state[board[i][j]] += Corner_Two_Sides;
					else {
						if(disc_count[BLACK] + disc_count[WHITE] >= Latter_Quarter_Threshold)score_cur_situation_state[board[i][j]] += Edge_Mo_Qi;
						else score_cur_situation_state[board[i][j]] += Edge_Chu_Qi;
					}
				}
				...
				else score_cur_situation_state[board[i][j]] += Remainings;
			}
		}

		int total_how_many_can_flip = 0;
		if (disc_count[WHITE] + disc_count[BLACK] >= Latter_Quarter_Threshold) {//如果已經接近尾聲的話，
			//score_Mobi = score_Mobi * 4 / 5; //Mobi比較不那麼重要了
			for (int i = 0; i < how_many_can_flip.size(); i++)total_how_many_can_flip += how_many_can_flip[i];
		}
		
		int total_score = 0;
		if (cur_player == BLACK) {
			total_score += static_cast<double>(score_Mobi) * (static_cast<double>(disc_count[EMPTY]) / (SIZE*SIZE));
			total_score += score_cur_situation_state[BLACK];
			if (disc_count[WHITE] + disc_count[BLACK] >= Latter_Quarter_Threshold)total_score += static_cast<double>(total_how_many_can_flip) * Flipping * ((static_cast<double>(disc_count[WHITE]) + disc_count[BLACK])*1.5 / (SIZE*SIZE));
			else total_score += static_cast<double>(total_how_many_can_flip) * Flipping* ((static_cast<double>(disc_count[WHITE]) + disc_count[BLACK])*0.3 / (SIZE*SIZE));
			total_score -= score_cur_situation_state[WHITE];
		}
