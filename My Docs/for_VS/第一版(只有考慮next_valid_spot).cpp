...
void write_valid_spot(std::ofstream& fout) {
	...
	OthelloBoard curState(board, player);//先把目前main.exe所已經提供的盤面建構出來。題目跟我講我現在要下的是player(ex: 這裡以及以下的例子都用我想要下白色來說明)
...
int OthelloBoard::MiniMax(int cur_depth, int terminal_depth) {
	...
	if (cur_depth == terminal_depth) {
		...
		int num_of_corners_inner = 0;
		int num_of_corners_two_sides = 0;
		int num_of_remainings = 0;
		for (int i = 0; i < next_valid_spots.size(); i++) {
			if (next_valid_spots[i] == Corners[0] || next_valid_spots[i] == Corners[1] || next_valid_spots[i] == Corners[2] || next_valid_spots[i] == Corners[3])
				...
				num_of_corners_two_sides++;
			else num_of_remainings++;
		}
		
		if (cur_player == BLACK) {//分數越高代表對BLACK越好
			score += num_of_corners * 100 + num_of_corners_two_sides * 10 - num_of_corners_inner * 15 + num_of_remainings;
		}
		else if (cur_player == WHITE) {//分數越低代表對WHITE越好
			score -= num_of_corners * 100 + num_of_corners_two_sides * 10 - num_of_corners_inner * 15 + num_of_remainings;
		}
